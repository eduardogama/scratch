...
from dash_emulator.models import MPD, AdaptationSet, SegmentTemplate, Representation, Segment
...
class DefaultMPDParser(MPDParser):
    ...
    media_presentation_duration: float = 0.0
    ...
    def parse(self, content: str, url: str) -> MPD:
        ...
        DefaultMPDParser.media_presentation_duration = self.parse_iso8601_time(root.attrib.get("mediaPresentationDuration", ""))
        ...
        for adaptation_set_xml in period:
            if adaptation_set_xml.attrib.get("mimeType") != "audio/mp4":
        ...
        return MPD(content, url, type_, DefaultMPDParser.media_presentation_duration, max_segment_duration, min_buffer_time,
                   adaptation_sets)

    def parse_adaptation_set(self, tree: Element, base_url) -> AdaptationSet:
        id_ = tree.attrib.get("id") or "1"
        ...
        segment_template_xml: Element = tree.find("SegmentTemplate")

        if segment_template_xml is None:
            raise MPDParsingException("The MPD support is not complete yet")
        

        segment_template = self.parse_segment_template(segment_template_xml, base_url)
        tree.remove(segment_template_xml)

        representations = {}
        for representation_tree in tree:
            representation_tree.tail = segment_template_xml

            representation = self.parse_representation(representation_tree, base_url)
            representations[representation.id] = representation

        return AdaptationSet(int(id_), content_type, frame_rate, max_width, max_height, par, representations, segment_template)

    def parse_segment_template(self, tree: Element, base_url) -> SegmentTemplate:
        duration = float(tree.attrib['duration'])
        timescale = float(tree.attrib['timescale'])
        startNumber = int(tree.attrib['startNumber'])
        initialization = tree.attrib['initialization']
        media = tree.attrib['media']

        return SegmentTemplate(
            duration=duration/timescale, timescale=timescale, media=media, startNumber=startNumber, initialization=initialization 
        )
        
    def parse_representation(self, tree: Element, base_url) -> Representation:
        segment_template = tree.find("SegmentTemplate")

        if segment_template is not None or tree.tail.tag == "SegmentTemplate":
    ...
    def parse_representation_with_segment_template(tree: Element, base_url) -> Representation:
        ...
        segment_template: Element = tree.find("SegmentTemplate") or tree.tail
        ...
        if segment_timeline is not None:
            ...
            while timestamp < DefaultMPDParser.media_presentation_duration:
                url = base_url + media.replace("$Number$", str(num))
                segments.append(Segment(url, duration))
                timestamp += duration / timescale
                num += 1

        return Representation(id_, mime, codec, bandwidth, width, height, initialization, segments)

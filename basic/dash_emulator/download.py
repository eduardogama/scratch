...
class DownloadManagerImpl(DownloadManager):
    ...
    async def download_segment(self, url) -> Optional[bytes]:
    ...
    async def download(self, url, save=False, auxstr = "Start downloading") -> Optional[bytes]:
        self._busy = True
        self._stop = False
        self.log.info("%s %s" % (auxstr, url))

        if self._session is None:
            session_timeout = aiohttp.ClientTimeout(total=5, sock_read=5, sock_connect=5)
        ...
        try:
            ...
            await self.download(url, auxstr="Try again downloading")
        except Exception as e:
            print ("excepetion error", e)
            
            await self.close()
            self._session = None
            
            await self.download(url, auxstr="Try again downloading after session close")
